...
FORMAT_VERSION = b'\x83' #struct.pack("b", 131)
...
def decode(binary):
    """
        ...
        {'error': None, 'payload': {'active_param': 1, 'pipe_before': False}, 'signatures': [{'docs': 'docs', 'name': 'name', 'params': ['list']}, {'docs': 'snd doc', 'params': ['list']}], 'request_id': 1}
    """
    if not bytes.startswith(binary, FORMAT_VERSION):
...
def __decode_func(binary):
    if bytes.startswith(binary, SMALL_INTEGER_EXT):
        return (2, __decode_int)
    elif bytes.startswith(binary, INTEGER_EXT):
        return (5, __decode_int)
    elif bytes.startswith(binary, BINARY_EXT):
        (size, ) = struct.unpack(">L", binary[1:5])
        return (1 + 4 + size, __decode_string)
    elif bytes.startswith(binary, ATOM_EXT):
        (size, ) = struct.unpack(">H", binary[1:3])
        return (1 + 2 + size, __decode_atom)
    elif bytes.startswith(binary, NIL_EXT):
        return (1, __decode_list)
    elif bytes.startswith(binary, LIST_EXT):
    ...
    elif bytes.startswith(binary, MAP_EXT):
...
def __decode_int(binary):
    ...
    """
    if bytes.startswith(binary, SMALL_INTEGER_EXT):
        num = int.from_bytes(binary[1:], byteorder='big')
        return num
    num = int.from_bytes(binary[1:], byteorder='big', signed=True)
    return num
...
def __encode_map(obj):
    ...
    for k,v in obj.items():
        b = b'%s%s%s' % (b, __encoder_func(k)(k), __encoder_func(v)(v))
    return MAP_EXT + b
...
def __encode_string(obj):
    ...
    """
    return BINARY_EXT + struct.pack(">L", len(obj)) + str.encode(obj)
...
def __encode_atom(obj):
    return ATOM_EXT + struct.pack(">H", len(obj)) + (str.encode(obj))
...
def __encode_int(obj):
    ...
    """
    if 0 <= obj <= 255:
        return SMALL_INTEGER_EXT + int.to_bytes(obj, 1, byteorder='big')
    elif -2147483648 <= obj <= 2147483647:
        return INTEGER_EXT + int.to_bytes(obj, 4, byteorder='big', signed=True)
    else:
        raise "Maybe later"
